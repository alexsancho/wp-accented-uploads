...
class PluginTest extends WP_UnitTestCase {
	...
	function test_shouldnt_sanitize_ascii() {
		...
		$this->assertEquals( $file, Asancho\Sanitizer::remove_accents( $file ) );
	}

	function test_should_sanitize_accent_with_url() {
		$file = 'http://example.com/wp-content/uploads/2020/02/ääkkönen.jpg';

		$this->assertEquals( 'http://example.com/wp-content/uploads/2020/02/aakkonen.jpg', Asancho\Sanitizer::remove_accents( $file ) );
	}

	function test_should_sanitize_accent_with_path() {
		$file = '/data/uploads/2020/02/ääkkönen.jpg';

		$this->assertEquals( '/data/uploads/2020/02/aakkonen.jpg', Asancho\Sanitizer::remove_accents( $file ) );
	}

	function test_should_sanitize_accent() {
		$file = 'ääkkönen.jpg';

		$this->assertEquals( 'aakkonen.jpg', Asancho\Sanitizer::remove_accents( $file ) );
	}

	function test_should_sanitize_special_chars_and_spaces() {
		$file = 'mathematical file of ¼.png';

		$this->assertEquals( 'mathematical-file-of-.png', Asancho\Sanitizer::remove_accents( $file ) );
	}
	...
	function test_should_create_nfc_encoding_errors() {
		...
		$error_file = Asancho\Sanitizer::replace_filename_with_encoding_errors( $correct_file );
